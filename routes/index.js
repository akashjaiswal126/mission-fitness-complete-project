...
route.get('/cart/remove/:id',(req,res,next)=>{
...
const mailOptions = {
    from: `"Mission Fitness and Food Supplements" <${email}>`, // sender address
    to: 'akashjaiswal126@gmail.com', // list of receivers
    subject: "Mission Fitness Customer Enquiry", // Subject line
    generateTextFromHTML: true,
    html: output, // html body
};
transporter.sendMail(mailOptions,(error,info)=>{
    if(error){
        ...
        console.log('Mail sent %s',info.response);
        req.flash('success_msg','You have sent your Query/Suggestion Successfully !')
        res.redirect('/contact');
    }
    res.redirect('/contact');
})
...
route.get('/order',(req,res,next)=>{
    ...
    const attachments = parsedImage.map((file)=>{
        return { filename: file.item.image, path: `./public/images/${file.item.image}`,cid : `image1@${file.item.image}` };
      });
    ...
    <p style="text-align:justify">Thank you for your order. We’ll send a confirmation when your order ships. Your estimated delivery date is indicated below. If you would like to view the status of your order or make any changes to it, please visit Your Orders on <span style="color:#c24d0a"> missionFitness.in</span></p>
    <hr>
    <h3>Order Details: -</h3>
   ...
   <hr>
   <p>Order id: #<span style="color:#405c91">${req.user.id} </span></p>
   <Placed on Saturday, May 22, 2021>
   <table width="50%">
   <tr>
   <td>Item Subtotal:</td>
   ...
   <hr>
<br>
<p> If you use a mobile device, you can receive notifications about the delivery of your package and track it from our free<span style="color:#405c91";> missionFitness </span>app.<p>
...
`
 ...
 // send mail with defined transport object
 const mailOptions = {
     from: `"Mission Fitness Order Confirmation" <${req.user.email}>`, // sender address
     to: `${req.user.email},akashjaiswal126@gmail.com`, // list of receivers
     subject: "Mission Fitness Your Order is placed Successfully", // Subject line
     generateTextFromHTML: true,
     html: output, // html body
 ...
 transporter.sendMail(mailOptions,(error,info)=>{
     if(error){
         ...
         console.log('Mail sent %s',info.response);
         delete req.session.cart
         req.flash('success_msg','We have Received Your Order and We will get back to you soon !')
         res.redirect('/cart');
     }
     res.redirect('/cart');
 })
